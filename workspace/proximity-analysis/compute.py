...
def compute_cell_cell_interactions(df, radius_in_micrometer=15, pixel_size=0.5, x_col='Centroid X', y_col='Centroid Y', phenotype_col='Phenotype', sample_col='imageid', parent_region_col='Parent Region', parent_area_col='Parent Area µm^2'):
    ...
    for sample in sample_list:
        ...
        sample_df = df[df[sample_col] == sample].copy().reset_index(drop=True)
        parent_regions = sample_df[[parent_region_col, parent_area_col]].drop_duplicates()
        total_area_um2 = parent_regions[parent_area_col].sum()
        total_area_mm2 = total_area_um2 / 1e6

        coords = sample_df[[x_col, y_col]].values
        phenotypes = sample_df[phenotype_col].values
        tree = KDTree(coords)
        ...
        matrix.to_csv(f"interaction_matrix_raw/{sample_slug}.csv")


df = pd.concat([pd.read_csv(f) for f in inputs], ignore_index=True)
