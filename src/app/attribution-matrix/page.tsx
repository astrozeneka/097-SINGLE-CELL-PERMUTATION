...
export default function AttributionMatrix() {
  const [file, setFile] = useState<File | null>(null);
  const [markers, setMarkers] = useState<string>("CD68,CD163,CD206,CD11c,PDL1,CD3,CD4,Foxp3,CD8,PD1,CD20,CD31,Ki67");
  const [phenotypes, setPhenotypes] = useState<string>("TAM,DC,T,Helper T,T PD1,Tregs,Endothelial cells,Ki67 proliferating cells");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Run attribution matrix with SSE
      setLogs((prev) => [...prev, "Starting attribution matrix generation..."]);
      ...
      const eventSource = new EventSource(`/api/run-attribution-matrix?${params.toString()}`);
        ...
        <form onSubmit={handleSubmit} className="mb-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="CD68,CD163,CD206,CD11c,PDL1,CD3,CD4,Foxp3,CD8,PD1,CD20,CD31,Ki67"
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1.5 uppercase tracking-wider">
              Phenotypes (comma-separated)
            </label>
            <input
              type="text"
              value={phenotypes}
              onChange={(e) => setPhenotypes(e.target.value)}
              placeholder="TAM,DC,T,Helper T,T PD1,Tregs,Endothelial cells,Ki67 proliferating cells"
              className="w-full px-3 py-1.5 bg-slate-900 text-slate-200 text-sm border-b border-slate-700 focus:border-slate-500 focus:outline-none transition-colors"
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="bg-slate-900/50 backdrop-blur border-l border-slate-800 h-[calc(100vh-480px)] overflow-auto flex items-center justify-center p-3">
              {!isProcessing && outputFilename ? (
                <img
                  src={`/api/download-file?filename=${outputFilename}`}
                  alt="Attribution Matrix Heatmap"
                  className="max-w-full h-auto"
                />
              ) : (
                <div className="text-slate-600 italic">No image generated yet...</div>
              )}
            </div>
          </div>
        </div>
